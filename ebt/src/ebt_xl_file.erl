...
list_dir(Dir) -> ebt_xl_io:apply_io(file, list_dir, [Dir]).
...
ensure_dir(Path) -> ebt_xl_io:apply_io(filelib, ensure_dir, [Path]).
...
read_file(Path) -> ebt_xl_io:apply_io(file, read_file, [Path]).
write_file(Path, Data) ->
    case ensure_dir(Path) of
        ok -> ebt_xl_io:apply_io(file, write_file, [Path, Data]);
        E -> E
    end.
open(File, Mode) -> ebt_xl_io:apply_io(file, open, [File, Mode]).
...
delete(Path) ->
    case type(Path) of
        ...
        {ok, directory} ->
            do([error_m ||
                Files <- list_dir(Path),
                ebt_xl_lists:eforeach(fun(P) -> delete(filename:join(Path, P)) end, Files),
                ebt_xl_io:apply_io(file, del_dir, [Path])
            ]);
...
using(File, Mode, F) -> ebt_xl_auto:using(?MODULE, [File, Mode], F).
